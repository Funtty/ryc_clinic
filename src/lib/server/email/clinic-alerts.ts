 ...
 */
export async function notifyClinic(topic: string, rows: ClinicAlertRow[]): Promise<boolean> {
  const recipient = env.CLINIC_NOTIFY_EMAIL?.trim();
  ...
  try {
    ...
    if (!res.ok) {
      console.warn(`[clinic-alert] Resend ${res.status}: ${await res.text()}`);
      return false;
    }
    return true;
  } catch (err) {
